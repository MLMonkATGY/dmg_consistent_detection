...
def combine(tt1, tt2, output_file):
    ...
    for i in temp:
        assert not (i in temp2), (
            "Duplicate filenames detected between the two files! @" + i
        )
        ...
        if c in d2_categories_names:
            ...
            if d1_categories_names[c] != d2_categories_names[c]:
                assert (
                    ...
                    c, d1_categories_names[c], d2_categories_names[c]
                )
        else:
            assert False, "Category name: {} in file 1 does not exist in file 2".format(
                c
            )
    ...
    for c in d2_categories_names:
        if c in d1_categories_names:
            ...
            assert False, "Category name: {} in file 2 does not exist in file 1".format(
                c
            )
    ...
    added, removed, modified, same = dict_compare(
        files_check_classes, files_check_classes_temp
    )
    assert len(added) == 0 and len(removed) == 0 and len(modified) == 0, (
        "filenames detected before merging error: "
        + len(added)
        + " filenames added "
        + len(removed)
        + " filenames removed "
        + len(modified)
        + " filenames' classes modified "
        + len(same)
        + " filenames entries reserved"
    )
    ...
    assert len(added) == 0 and len(removed) == 0 and len(modified) == 0, (
        "filenames detected after merging error: "
        + len(added)
        + " filenames added "
        + len(removed)
        + " filenames removed "
